async function loadAllBookCovers(libros) {
    console.log(`Iniciando búsqueda de portadas para ${libros.length} libros sin imagen propia...`);
    
    libros.forEach(libro => {
        const statusElement = document.getElementById(`cover-status-${libro.id}`);
        if (statusElement) {
            statusElement.textContent = 'Buscando portada...';
        }
    });
    ...
    try {
        ...
        booksWithCovers.forEach(book => {
            ...
            const statusElement = document.getElementById(`cover-status-${book.id}`);
            ...
            if (imgElement && statusElement) {
                if (book.coverUrl) {
                    ...
                    // Manejar error de carga de la imagen externa
                    imgElement.onerror = () => {
                        imgElement.src = '../img/no-cover.jpg';
                        statusElement.textContent = 'Portada no disponible';
                    };
                } else {
                    // No se encontró portada
                    imgElement.src = '../img/no-cover.jpg';
                    statusElement.textContent = 'Portada no disponible';
                }
            }
        ...
        console.log('Búsqueda de portadas completada');
        
    } catch (error) {
        console.error('Error general en la búsqueda de portadas:', error);
        
        // En caso de error general, actualizar todos los elementos
        libros.forEach(libro => {
            const imgElement = document.getElementById(`book-${libro.id}`);
            const statusElement = document.getElementById(`cover-status-${libro.id}`);
            
            if (imgElement && statusElement) {
                imgElement.src = '../img/no-cover.jpg';
                statusElement.textContent = 'Error buscando portada';
            }
        });
    }
...
class BookCoverService {
    constructor() {
        this.timeout = 11000; // 11 segundos de timeout
    }
...
async function mostrarLibros(datos) {
    ...
    if (datos.success && datos.count > 0) {
                ...
                <tr>
                    ...
                    <td>
                        <img id="book-${libro.id}" 
                             src="../img/peques/${libro.imagen || 'no-cover.jpg'}?${new Date().getTime()}" 
                             alt="${libro.titulo}" 
                             style="max-width: 100px; height: auto;"
                             onerror="handleImageError(this, ${libro.id})" />
                        <div id="cover-status-${libro.id}" class="cover-status" style="font-size: 12px; margin-top: 5px; color: #666;">
                        </div>
                    </td>
        ...
        // Buscar portadas en APIs externas para libros sin imagen propia
        const librosSinImagenPropia = libros.filter(libro => 
            !libro.imagen || 
            libro.imagen.trim() === '' || 
            libro.imagen === 'no-cover.jpg'
        );
        
        if (librosSinImagenPropia.length > 0) {
            loadAllBookCovers(librosSinImagenPropia);
        }
...
function handleImageError(imgElement, bookId) {
    const statusElement = document.getElementById(`cover-status-${bookId}`);
    
    // Si la imagen falló al cargar, buscar en APIs externas
    if (!imgElement.dataset.apiSearched) {
        imgElement.dataset.apiSearched = 'true';
        ...
        const libro = librosData.find(lib => lib.id == bookId);
        if (libro) {
            searchSingleBookCover(libro);
        }
    } else {
        // Ya se buscó en APIs, mostrar imagen por defecto
        imgElement.src = '../img/no-cover.jpg';
        if (statusElement) {
            statusElement.textContent = 'Portada no disponible';
        }
...
// Función para buscar portada de un solo libro
async function searchSingleBookCover(libro) {
    try {
        const coverUrl = await coverService.getCover(libro.titulo, libro.autor);
        const imgElement = document.getElementById(`book-${libro.id}`);
        const statusElement = document.getElementById(`cover-status-${libro.id}`);
        ...
        if (imgElement && statusElement) {
            if (coverUrl) {
                ...
                // Si esta imagen externa también falla
                imgElement.onerror = () => {
                    imgElement.src = '../img/no-cover.jpg';
                    statusElement.textContent = 'Portada no disponible';
                };
        ...
        console.log(`Error buscando portada para "${libro.titulo}":`, error);
        const imgElement = document.getElementById(`book-${libro.id}`);
        const statusElement = document.getElementById(`cover-status-${libro.id}`);
...
function enviarDatosNuevoLibro(e) {
    ...
    if (modoEdicion) {
        metodo = 'PUT'; // o 'PATCH' según tu API
